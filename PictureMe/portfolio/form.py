...
class OrderFrom(forms.ModelForm):
    purpose1 = forms.ChoiceField(widget=forms.Select, choices=Order.type_choices)
    ...
    class Meta:
        ...
        widgets = {
            ...
            "email": forms.TextInput(
                attrs={
                    "type": "email",
                    "placeholder": "komolov.timurka@mail.ru",
                    "required": "required",
                    "id": "mail",
                }
            ),
            ...
            "is_vk_answer": forms.CheckboxInput(
                attrs={
                    "id": "is_vk_answer",
                }
            ),
    ...
    def __str__(self):
        ...
        purpose_label = dict(self.fields["purpose1"].choices).get(
            self.cleaned_data["purpose1"]
        )
        return (
            f"Поступило сообщение в {datetime.datetime.now().strftime('%H:%M:%S %d:%m:%Y')}\n"
